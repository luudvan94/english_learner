...
enum Direction { nextPage, previousPage, nextWord, previousWord, none }
...
class _WordContentState extends State<WordContent>
  ...
  Direction shouldChange(Offset startPoint, Offset currentPoint) {
    ...
    if (alPhaX.abs() > alPhaY.abs() * 2) {
      if (this.dragFromRight) {
        ...
        return alPhaX.abs() >= MediaQuery.of(context).size.width / 5
            ? Direction.nextWord
            : Direction.none;
      } else {
        if (startPoint.dx > controlPoint.dx) {
        ...
        return currentPoint.dx >= MediaQuery.of(context).size.width / 5
    ...
    if (alPhaY < 0 && alPhaY.abs() > MediaQuery.of(context).size.height / 10) {
      return Direction.nextPage;
        ...
        alPhaY.abs() > MediaQuery.of(context).size.height / 10) {
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    setState(() {
      prevPoint = details.globalPosition;
      controlPoint =
          Offset(controlPoint.dx + dxOffset, details.globalPosition.dy);
    });
    // }
  }
  ...
  Container firstPageView(Word word) {
    content() {
      ...
      contentWidgets
          .add(ElementStyle.definition(word.definitions[0].definition, false));
  ...
  Container nextPageView(Definition defi, Word word) {
    ...
    contentWidgets.add(new SizedBox(
      child: new Column(children: examples),
    ));
    return Container(
        child: Container(
    ...
    return GestureDetector(
      ...
      child: ClipPath(
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          ElementStyle.page((currentPage + 1).toString(),
                              (widget.word.definitions.length + 1).toString()),
                        ],
                      ),
                    ),
                    Expanded(
        ...
        clipper: CustomBackground(
            controlPoint: controlPoint, fromRight: dragFromRight),
      ),
    );
  }
}
...
class CustomBackground extends CustomClipper<Path> {
  CustomBackground(
      ...
      this.controlPoint = const Offset(0, 0),
  ...
  Path getClip(Size size) {
    ...
    if (this.fromRight && isControlPointChanged()) {
      ...
      path.cubicTo(firstControlPoint.dx, firstControlPoint.dy,
          secondControlPoint.dx, secondControlPoint.dy, 0, 0);
    }
